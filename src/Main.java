...
public class Main {
    ...
    private static void fourthAssignment() {
        double average;
        System.out.print("Average salary:");
        average = assignmentList()
                .stream()
                .map(Employee::getSalary)
                .mapToDouble(salary -> salary)
                .sum();
        average /= assignmentList().stream().count();
        System.out.println(average);
    }

    private static void fifthAssignment() {
    ...
    private static void sixthAssignment() {
        System.out.print("Employee ages grater than 25: ");
        boolean result = assignmentList()
                .stream()
    ...
    private static void seventhAssignment() {
    ...
    private static void eighthAssignment() {
    ...
    private static void ninthAssignment() {
    ...
    private static void tenthAssignment() {
    ...
    private static void eleventhAssignment() {
        System.out.println("Partition employees into employers and non-employers");
        assignmentList().stream().collect(Collectors.groupingBy(Employee::isEmployer))
                .forEach((key, value) -> {
    ...
    private static void twelfthAssignment() {
        System.out.print("Total salary is ");
        double total = assignmentList().stream().map(Employee::getSalary).mapToDouble(salary -> salary).sum();
        System.out.println(total);
    }

    private static void thirteenthAssignment() {
        System.out.println("Employee with the lowest salary:");
        assignmentList().stream().sorted(Comparator.comparing(Employee::getSalary))
                .limit(1).forEach(System.out::print);
    }

    private static void fourteenthAssignment() {
        System.out.println("List of the unique surnames of all employees:");
        assignmentList().stream().map(Employee::getName)
                .collect(Collectors.toSet()).forEach(System.out::print);
    }

    private static void fifteenthAssignment() {
        System.out.println("Map of employee names to their respective salaries(lowest is first)");
        assignmentList().stream().sorted(Comparator.comparing(Employee::getSalary))
                .map(Employee::getName)
                .forEach(s -> System.out.println(s + " ,"));
    }

    private static void sixteenthAssignment() {
        System.out.println("List the names of employees who are not employers:");
        assignmentList().stream().filter(employee -> !employee.isEmployer())
                .map(Employee::getName)
                .forEach(System.out::print);
    }

    private static void seventeenthAssignment() {
        System.out.println("List of employees' full names (name + surname):");
        assignmentList().stream()
                .map(employee -> employee.getName() + " " + employee.getSurname())
                .forEach(System.out::print);
    }

    private static void eighteenthAssignment() {
    ...
    private static void nineteenthAssignment() {
        double sum = assignmentList().stream()
                .filter(Employee -> Department.IT.equals(Employee.getDepartment()))
                .map(Employee::getSalary)
                .mapToDouble(salary -> salary)
                .sum();
    ...
    private static void twentyAssignment() {
    ...
    private static List<Employee> assignmentList() {
